...
function MapTabInner({ T, sT, P, setP, inpData, setInpData, setTab, isActive, setSelMap, autoAreasPoly, setAutoAreasPoly, isExport }) {
  ...
  useEffect(() => {
    ...
    const handleAutoAreas = () => {
        if (!selTramos || selTramos.length === 0) {
            alert("Primero debes seleccionar los tramos a los que quieres generarles áreas aferentes.");
            return;
        }
        ...
        setTimeout(() => {
            try {
                let radioMeters = radio || 40;
                let featuresToBuffer = [];
                selTramos.forEach(st => {
                    if (!st || !st.coords || st.coords.length < 2) return;
                    let lineCoords = st.coords.map(c => [c[1], c[0]]);
                    let line = turf.lineString(lineCoords);
                    line.properties = { tramoId: st.id || st.de + "-" + st.a, pozoId: st.de };
                    featuresToBuffer.push(line);
                });
                
                let bufferedPolys = featuresToBuffer.map(f => {
                    let buf = turf.buffer(f, radioMeters / 1000, {units: 'kilometers'});
                    buf.properties = { ...f.properties, areaHa: turf.area(buf) / 10000 };
                    return buf;
                });
                
                if (setAutoAreasPoly) setAutoAreasPoly(bufferedPolys);
                setIsCalculating(false);
            } catch (err) {
                console.error("Error auto-areas:", err);
                setIsCalculating(false);
            }
        }, 100);
    };
    ...
    const handleCalcularAreas = (modoSeleccionado) => {
        if (cartografia.length === 0) {
            alert("Debes cargar una cartografía base (.zip) primero.");
            return;
        }
      ...
      if (pozosToDraw.length > 0 && tramosToDraw.length > 0) {
        ...
        rData.forEach(r => {
            ...
            if (s && e && s.lat !== undefined && s.lng !== undefined && e.lat !== undefined && e.lng !== undefined && !isNaN(s.lat) && !isNaN(e.lat)) {
              ...
              let diamValRaw = String(r.diametroCom || r.diametro || "200").trim();
              let diamVal = formatDiam(diamValRaw, matVal);
              pl.bindTooltip("<b>Tramo:</b> " + r.de + " - " + r.a + "<br><b>Longitud:</b> " + Number(lengthVal).toFixed(2) + " m<br><b>Pendiente:</b> " + Number(slopeVal).toFixed(4) + " m/m<br><b>Diámetro:</b> " + diamVal + "<br><b>C.Ras.DE:</b> " + r.cotaRasante.toFixed(2) + "<br><b>C.Ras.A:</b> " + r.cotaRasanteA.toFixed(2) + "<br><b>C.Fon.DE:</b> " + r.cotaFondo.toFixed(2) + "<br><b>C.Fon.A:</b> " + r.fondo_final.toFixed(2), { permanent: false, sticky: true });
              pl.on('click', (evt) => {
  ...
  const exportToApp = (onlySelection = false) => {
        ...
        try {
            ...
            for (let f of geoJsonData.features) {
                if (turf.booleanPointInPolygon(pt, f)) {
                    return f.properties.DENS2008 || f.properties.DENSIDAD || f.properties.Densidad || f.properties.densidad;
                }
            }
        } catch(e){}
        return null;
        ...
        for (let lin of lineas) {
            ...
            if (coordsMatch) {
                ...
                if (points.length > 2) {
                    ...
                    try {
                        let poly = turf.polygon([points]);
                        if (turf.booleanPointInPolygon(pt, poly)) return val;
                    } catch(e) {}
                }
            }
        }
    ...
    itemsToProcess.forEach(t => {
        ...
        if (!foundGlobalDen && tDen !== undefined) foundGlobalDen = parseFloat(tDen);
    ...
    let processed = itemsToProcess.map((t, index) => {
        ...
        if (calcT.areaPoli && calcT.areaPoli.geometry && calcT.areaPoli.geometry.coordinates) {
            areaFeature = calcT.areaPoli;
        } else if (typeof autoAreasPoly !== 'undefined' && autoAreasPoly && autoAreasPoly.length > 0) {
            let found = autoAreasPoly.find(f => f.properties && String(f.properties.de) === String(t.de) && String(f.properties.a) === String(t.a));
        ...
        let dStr = String(calcT.diametro || t.diametroCom || "315").trim();
        let dRaw = dStr.replace(/[^0-9.]/g, "");
        let dMM = +dRaw;
        if(dStr.includes('"') || dMM < 50) dMM = Math.round(dMM * 25.4);
        if(dMM < 100) dMM = 315;
        let dNom = dMM + " mm";
    ...
    const handleInjectToCalc = () => {
        let itemsToProcess = (P && P.tramos && P.tramos.length > 0) ? P.tramos : T;
        let newVerts = [];
        let newPozosMap = {};
        let newTr = [];
        let injectedAreas = new Set();
        let foundGlobalEst = null;
        let foundGlobalDen = undefined;
        let foundGlobalCon = undefined;

        // Mantener pozos existentes primero
        if (inpData && inpData.pozos) {
            inpData.pozos.forEach(pz => {
                let key = pz.IdNodo || pz.Nombre;
                if (newPozosMap[key]) {
                    newPozosMap[key] = { ...pz, ...newPozosMap[key] };
        ...
        let processed = itemsToProcess.map((t, index) => {
            ...
            if (pts.length === 0) {
                let oldTr = ((inpData && inpData.tr) || []).find(rt => (String(rt.DE1 || rt.DE || rt.de)===String(t.de)) && (String(rt.A1 || rt.A || rt.a)===String(t.a)));
                if (oldTr && oldTr.coords) pts = oldTr.coords;
            }
            if (pts.length > 0) {
                let ptMFirst = projectToMeters(pts[0][1], pts[0][0]);
                let oldPzDE = ((inpData && inpData.pozos) || []).find(p => String(p.IdNodo || p.Nombre) === String(t.de)) || {};
                newPozosMap[t.de] = { ...oldPzDE, IdNodo: t.de, Nombre: t.de, CoordX: ptMFirst[0], CoordY: ptMFirst[1] };

                let ptMLast = projectToMeters(pts[pts.length - 1][1], pts[pts.length - 1][0]);
                let oldPzA = ((inpData && inpData.pozos) || []).find(p => String(p.IdNodo || p.Nombre) === String(t.a)) || {};
                newPozosMap[t.a] = { ...oldPzA, IdNodo: t.a, Nombre: t.a, CoordX: ptMLast[0], CoordY: ptMLast[1] };
                let oldTr = ((inpData && inpData.tr) || []).find(rt => (String(rt.DE1 || rt.DE || rt.de)===String(t.de)) && (String(rt.A1 || rt.A || rt.a)===String(t.a))) || {};
                newTr.push({ ...oldTr, id_1: (index + 1), id: (index + 1), de: t.de, a: t.a, coords: pts });
            }
            
            let calcT = selTramos.find(st => String(st.de) === String(t.de) && String(st.a) === String(t.a)) || t;
            let areaFeature = null;
            if (calcT.areaPoli && calcT.areaPoli.geometry && calcT.areaPoli.geometry.coordinates) {
                areaFeature = calcT.areaPoli;
            } else if (typeof autoAreasPoly !== 'undefined' && autoAreasPoly && autoAreasPoly.length > 0) {
            ...
            if (areaFeature) {
               ...
               if (geom.type === 'MultiPolygon') {
                   geom.coordinates.forEach((polyCoords, pIdx) => {
                       let rings = polyCoords;
                       if (rings && rings.length > 0) {
                           rings[0].forEach(pt => {
                               let ptM = projectToMeters(pt[0], pt[1]);
                               newVerts.push({ IDNODO: t.de, DE: String(t.de).trim() + "-" + String(t.a).trim(), SubName: "S" + (index + 1) + "_" + pIdx, CoordX: ptM[0], CoordY: ptM[1] });
                           });
                       }
                   });
               } else {
            ...
            let dNom = formatDiam(calcT.diametro || t.diametroCom || "315", matVal);
            ...
            let tDen = (calcT.densidad !== undefined) ? calcT.densidad : t.densidad;
            let tCon = (calcT.consumo !== undefined) ? calcT.consumo : t.consumo;
            let tEst = (calcT.estacion !== undefined) ? calcT.estacion : t.estacion;
            
            if (tEst && !foundGlobalEst) foundGlobalEst = String(tEst).trim();
            if (tDen !== undefined && foundGlobalDen === undefined) foundGlobalDen = parseFloat(tDen);
            if (tCon !== undefined && foundGlobalCon === undefined) foundGlobalCon = parseFloat(tCon);
            
            let globalDenToUse = foundGlobalDen !== undefined ? foundGlobalDen : ((P && P.densidad !== undefined) ? parseFloat(P.densidad) : 200);
            let globalConToUse = foundGlobalCon !== undefined ? foundGlobalCon : ((P && P.consumo !== undefined) ? parseFloat(P.consumo) : 140);
            let globalEstToUse = foundGlobalEst || (P && P.estacion) || "IDEAM";
        ...
        // Inyectar polígonos Thiessen huerfanos (ej. pozo final que no es inicio de ningún tramo)
        if (typeof bufferPoly !== 'undefined' && bufferPoly && bufferPoly.length > 0) {
            bufferPoly.forEach((feat, bIdx) => {
                let pid = String(feat.properties.id || feat.properties.IdNodo || feat.properties.Nombre || "");
                if (pid && !injectedAreas.has(pid)) {
                        ...
                        if (geom.type === 'MultiPolygon') {
                            geom.coordinates.forEach((polyCoords, pIdx) => {
                                ...
                                if (rings && rings.length > 0) {
                                    rings[0].forEach(pt => {
                                        let ptM = projectToMeters(pt[0], pt[1]);
                                        newVerts.push({ IDNODO: pid, DE: "", SubName: "S_" + pid + "_" + pIdx, CoordX: ptM[0], CoordY: ptM[1] });
                                    });
                                }
                            });
                        } else {
                            ...
                            if (rings && rings.length > 0) {
                                rings[0].forEach(pt => {
                                    let ptM = projectToMeters(pt[0], pt[1]);
                                    newVerts.push({ IDNODO: pid, DE: "", SubName: "S_" + pid, CoordX: ptM[0], CoordY: ptM[1] });
                                });
                            }
                        }
                    }
        ...
        <div style={{ display: 'flex', gap: '8px', marginTop: '16px', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button 
               ...
               style={{ 
                   flex: 1,
                   backgroundColor: isCalculating ? '#9ca3af' : '#8e44ad', 
                   color: 'white', 
                   fontWeight: 'bold', 
                   padding: '12px 8px', 
                   borderRadius: '12px', 
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <button 
                   onClick={handleAutoAreas}
                   disabled={isCalculating}
                   style={{ 
                       flex: 1,
                       padding: '12px',
                       backgroundColor: isCalculating ? '#e5e7eb' : '#10b981',
                       ...
                       boxShadow: isCalculating ? 'none' : '0 2px 4px rgba(16, 185, 129, 0.2)'
                   }}
                >
